    ...
    if request.method == 'POST' and request.FILES.get('archivo_dicom'):
        ...
        try:
            ...
            if os.path.exists(jpeg_file_path):
                ...
                return render(request, 'DREAMPP/index.html', {'imagen': jpeg_url , 'metadata' : metadata})
            else:
                return render(request, 'DREAMPP/index.html', {'error': 'Error al convertir el archivo DICOM a JPEG.'})

        except Exception as e:
            return render(request, 'DREAMPP/index.html', {'error': 'No se pudo procesar el archivo DICOM.'})
